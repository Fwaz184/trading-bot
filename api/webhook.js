...
async function runAnalysis(chatId) {
  ...
  try {
    ...
    console.error("API Response:", JSON.stringify(apiData));
    if (!result) {
      const errDetail = apiData.error ? apiData.error.message : "content فارغ";
      await sendMsg(chatId, "❌ خطأ API: " + errDetail, [[{ text: "🔄 تحليل جديد", callback_data: "new_analysis" }]]);
      return;
    }
